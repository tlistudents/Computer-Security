...
"""
The entire implementation here uses Professor Avi Kak's lecture 15
code as reference, most of the codes in this implmentation re-used
and modified the code from SHA256.py in lecture 15's code
"""
...
def sha512(InFile):
    ...
    #  Provided in page 43 on the lectre 15 note
    ...
    #  These are as provided on page 44-45 (page 44-45 of the PDF) of the NIST standard.  Note that 
    #  these are ONLY USED in STEP 3 of the hashing algorithm where we take each 1024-bit input message 
    #  block through 80 rounds of processing.
    ...
    # Turn content of input file into bv
    input = open(InFile, 'r')
    message = input.read()
    ...
    length1 = bv1.length()
    howmanyzeros = (896 - length1) % 1024 # now the block size is 1024 bit instead of 512
    zerolist = [0] * howmanyzeros
    bv2 = bv1 + BitVector(bitlist = zerolist)
    bv3 = BitVector(intVal = length, size = 128) # 128 bit for length
    bv4 = bv2 + bv3

    #  Initialize the array of "words" for storing the message schedule for a block of the input message:
    words = [None] * 80 # Now is 80 rounds instead of 64 rounds
    ...
    for n in range(0, bv4.length(), 1024):
        ...
        #  below(use lecture 15 note as reference when modify the sigma function, shifting value need 
        #  to be changed):
        ...
        # Step 3: Apply round-based processing to each input message block
        for i in range(80):
            ch = (e & f) ^ ((~e) & g)
            maj = (a & b) ^ (a & c) ^ (b & c)
            # Shifting value need to be changed
            sum_a = ((a.deep_copy()) >> 28) ^ ((a.deep_copy()) >> 34) ^ ((a.deep_copy()) >> 39) 
            sum_e = ((e.deep_copy()) >> 14) ^ ((e.deep_copy()) >> 18) ^ ((e.deep_copy()) >> 41)
            
            # bit-size changed to 64, so need to AND 16 F's(64 1's)
...
# Store the hash value to a file
def store(hash_hex_string, OutFile):
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        sys.stderr.write("Need two arguments\n")
        sys.exit(1)
    hash_hex_string = sha512(sys.argv[1])
    store(hash_hex_string, sys.argv[2])
