...
# Encryption:
def encrypt(inputMes,round_keys):
...
def ctr_aes_image(iv, image_file='image.ppm', out_file='enc_image.ppm', key_file='key.txt'):
    # Seprate the header
    with open(image_file, 'rb') as inputf:
        ppmAll = inputf.readlines()
    header = ppmAll[0:3] # first 3 lines
    header_bv = BitVector(size=0)
    for line in header:
        header_bv = header_bv + BitVector(rawbytes=bytes(line))
    data = b'' # empty byte
    for line in ppmAll[3:]: #rest of them are data
        data += line
    data_bv = BitVector(rawbytes=bytes(data))

    output = open(out_file, 'wb')
    header_bv.write_to_file(output)
    #print(header_bv.get_bitvector_in_hex())
    
    
    num_block = data_bv.size // 128  # number of blocks
    if (data_bv.size % 128):
        data_bv.pad_from_right(128 - (data_bv.size % 128))
        num_block += 1

    key1 = open(key_file,"r")
    key_bv = BitVector(textstring=key1.read())
    round_keys = [None for i in range(15)]
    key_words = gen_key_schedule_256(key_bv)
    for i in range(15):
    ...
    i = 0
    while i < num_block:
        # Get the block to encrypt
        bitVec = data_bv[i * 128: i * 128 + 128]

        # encrypt
        block_encrypt = encrypt(iv, round_keys)
        block_encrypt ^= bitVec
        
        #print(f': {block_encrypt.get_bitvector_in_hex()}')

        # update iv
        iv = BitVector(intVal=(iv.int_val() + 1), size=128)
        block_encrypt.write_to_file(output)
        i += 1
        #print(f'{i} / {num_block}')
    key1.close()
    output.close()
