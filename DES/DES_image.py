...
def encrypt(inputfile, key1, outputfile):
    ...
    # Just like the decrypt, has to count block manually
    i = 0 # starting from the first block
    num_block = data_bv.length() // 64  # number of blocks
    if (data_bv.length() % 64): 
        data_bv.pad_from_right(64 - (data_bv.length() % 64))  # padding 0
        num_block += 1
    while (i < num_block):
        bitvec = data_bv[i * 64: i * 64 + 64] #manually read 64 bit each time
        ...
        i += 1
    # Store new ppm file
    final_ppm = header_bv + out_bv
    with open(outputfile, 'wb') as outputf:
        final_ppm.write_to_file(outputf)

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Argument value need to be 3")
    else:
