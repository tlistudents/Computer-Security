...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    // ***********************************************
    // Change from strcpy() to strncpy() since they perform similar
    // task, becaue strcpy() is vulnerable to buffer overflow attck
    // as it never checks if the copied information size exceed the 
    // max amount the buffer can bear, but strncpy() fix it by set
    // a max amount of the data to put into the buffer, thus prevent
    // the buffer overflow
    // strcpy(str, recvBuff); // vulnerable to buffer overflow
    strncpy(str, recvBuff, MAX_DATA_SIZE); // not vulnerable to buffer overflow
    // ***********************************************
